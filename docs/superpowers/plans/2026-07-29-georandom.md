...
**Note — one required deviation from the code sample below (commit `c983034`):** the class must be declared `abstract class GeoRandomPointModel with _$GeoRandomPointModel` (not plain `class`). `mobile/pubspec.yaml` pins `freezed: ^3.2.3`, which generates truly-abstract getters in the `_$...` mixin — a non-abstract host class fails `dart analyze` with `non_abstract_class_inherits_abstract_member`. This also surfaced a pre-existing, out-of-scope repo issue: most other committed `*.freezed.dart` files under `mobile/lib/shared/models/` were generated by an older freezed 2.x-style generator and are NOT `abstract`; running `build_runner build` unscoped regenerates them into the new style and breaks their analysis too. Not fixed here (touches 7 unrelated files) — any future new freezed model in this repo needs `abstract` from the start.
