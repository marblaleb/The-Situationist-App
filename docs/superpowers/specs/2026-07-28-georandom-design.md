...
- `Infrastructure/Geo/IOverpassClient.cs` + `OverpassClient.cs` — `HttpClient` tipado (mismo patrón que `AnthropicClient`). **Actualizado durante implementación:** en vez de una sola query combinada, se hacen dos queries Overpass QL por celda (una para POIs — comercios y amenities, para densidad — y otra para polígonos de exclusión: `natural=water`, `landuse=military`, `leisure=nature_reserve`/`protected_area`, `building[access~"private|no"]`), priorizando claridad de parseo sobre un único round-trip. Ambas llamadas corren en paralelo (`Task.WhenAll`) para no duplicar la latencia, con verbosidad `skel` (sin tags, que el cliente no parsea) para minimizar payload, `User-Agent` propio, y reintento acotado (máx. 2 intentos) ante 504/429. `way["building"]` fue sacado de la query de densidad tras medir en producción ~168.000 elementos / 57MB / 16-19s en un área densa real, con 3 de 4 requests devolviendo 504 — los edificios diluían la señal comercio/amenity sin aportar contraste útil. Limitación conocida: la query de exclusión solo matchea `way[...]`, no `relation[...]` — zonas protegidas mapeadas en OSM como multipolígono (relation) no se detectan. Aceptable dado que el filtro ya está definido como "básico, no exhaustivo".
- `Infrastructure/Geo/GeoRandomCacheService.cs` — usa `IRedisCacheService` existente. Clave por celda geohash de precisión 5 (~4.9km × 4.9km). El fetch a Overpass se ancla al **centro decodificado de la celda** (no al punto exacto del request), con radio 9km — cubre la celda entera (semidiagonal ~3.46km) más el R máximo de usuario (5km) con margen, garantizando que cualquier request dentro de esa celda quede cubierto por el mismo fetch cacheado. TTL de 7 días — los POIs no cambian con frecuencia.
