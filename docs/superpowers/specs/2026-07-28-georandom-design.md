...
- `Infrastructure/Geo/IOverpassClient.cs` + `OverpassClient.cs` — `HttpClient` tipado (mismo patrón que `AnthropicClient`). Una sola query Overpass QL por celda que trae en un solo round-trip: POIs (comercios, amenities, edificios) para densidad, y polígonos de exclusión (`natural=water`, `landuse=military`, `leisure=nature_reserve`/`protected_area`, `building[access~"private|no"]`).
