...
public class AbandonDerivaSessionCommandHandlerTests
{
    ...
    public async Task ThrowsKeyNotFound_WhenSessionDoesNotExist()
    {
        ...
        var handler = new AbandonDerivaSessionCommandHandler(db);

        var act = () => handler.Handle(
            new AbandonDerivaSessionCommand(Guid.NewGuid(), Guid.NewGuid()),
    ...
    public async Task ThrowsKeyNotFound_WhenSessionBelongsToDifferentUser()
    {
        ...
        var act = () => handler.Handle(
            new AbandonDerivaSessionCommand(session.Id, Guid.NewGuid()), // UserId distinto
            CancellationToken.None);
    ...
    public async Task ThrowsInvalidOperation_WhenSessionIsNotActive(DerivaStatus status)
    {
        ...
        var act = () => handler.Handle(
            new AbandonDerivaSessionCommand(session.Id, userId),
            CancellationToken.None);
    ...
    [Fact]
    public async Task SetsStatusToAbandoned_WhenSessionIsActive()
    {
        await using var db = CreateDb();
        var userId = Guid.NewGuid();
        var session = ActiveSession(userId);
        ...
        var handler = new AbandonDerivaSessionCommandHandler(db);
        await handler.Handle(new AbandonDerivaSessionCommand(session.Id, userId), CancellationToken.None);

        var updated = await db.DerivaSessions.FindAsync(session.Id);
        updated!.Status.Should().Be(DerivaStatus.Abandoned);
    }

    [Fact]
    public async Task SetsEndedAt_WhenSessionIsAbandoned()
    {
        await using var db = CreateDb();
        var userId = Guid.NewGuid();
        var session = ActiveSession(userId);
